...
- Близко к ночной жизни Патонга`,
  ...
  {
    id: "gid://shopify/Product/7974403145782",
...
- Автобусы из Патонга и Карона`,
  ...
  {
    id: "gid://shopify/Product/7974403244086",
...
- Big C Phuket Town`,
  ...
  {
    id: "gid://shopify/Product/7974403604534",
...
- Tesco Lotus Patong`,
  ...
  {
    id: "gid://shopify/Product/7974403702838",
...
async function updateProduct(center) {
  const mutation = `
  ...
  try {
    ...
    if (data.data.productUpdate.userErrors.length > 0) {
      console.error(`❌ Ошибки обновления продукта ${center.title}:`, data.data.productUpdate.userErrors);
      return null;
    }

    console.log(`✅ Обновлен: ${center.title}`);
    ...
    console.error(`❌ Ошибка обновления ${center.title}:`, error.message);
    return null;
  }
}

async function updateAllShoppingCenters() {
  console.log('🚀 Обновляем все торговые центры в Shopify...\n');
  ...
  for (let i = 0; i < shoppingCenters.length; i++) {
    ...
    console.log(`\n📦 [${i + 1}/${shoppingCenters.length}] Обновляем: ${center.title}`);
    
    const result = await updateProduct(center);
  ...
  if (successCount > 0) {
    console.log('\n🎉 Торговые центры успешно обновлены в Shopify!');
