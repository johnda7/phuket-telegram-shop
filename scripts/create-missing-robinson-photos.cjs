// Создаем недостающие фотографии Robinson Lifestyle Phuket
// Используем рабочие URL для качественных фото
...
// Рабочие URL для недостающих фотографий
const missingPhotos = [
  {
...
async function createMissingRobinsonPhotos() {
  console.log('📥 СОЗДАЕМ НЕДОСТАЮЩИЕ ФОТОГРАФИИ ROBINSON LIFESTYLE PHUKET\n');
  ...
  for (let i = 0; i < missingPhotos.length; i++) {
    ...
    if (fs.existsSync(localPath)) {
      console.log(`📷 [${i + 1}/${missingPhotos.length}] ${photo.filename} - уже существует`);
      successCount++;
      continue;
    }
    
    console.log(`📷 [${i + 1}/${missingPhotos.length}] ${photo.filename}`);
    ...
    try {
      ...
      if (alternativeUrls[i]) {
        console.log(`   🔄 Пробуем альтернативный URL...`);
        try {
          await downloadFile(alternativeUrls[i], localPath);
          const fileSizeKB = (fs.statSync(localPath).size / 1024).toFixed(2);
  ...
  console.log('='.repeat(60));
  console.log(`✅ Успешно: ${successCount}/${missingPhotos.length}`);
  console.log(`❌ Ошибок: ${errorCount}/${missingPhotos.length}`);
...
createMissingRobinsonPhotos().catch(console.error);
